...
class LoginPage {
  ...
  constructor(page) {
    ...
    this.loginWithAutodeskButton = page.getByRole('button', { name: 'Login with Autodesk' });
    this.logoutButton = page.getByRole('button', { name: 'Logout from Autodesk' });
    this.selectPlatformButton = page.getByRole('button', { name: 'Go to platform selection' });
   ...
   * Useful for testing the logged-in state without going through OAuth
  ...
  /**
   * Mock the logged out state by clearing cookies
   * @returns {Promise<void>}
   */
  async mockLoggedOutState() {
