...
useEffect(() => {
  ...
  Promise.allSettled([
    ...
    .then((results) => {
      ...
      if (results[4].status === "fulfilled" && results[4].value && results[4].value.submittals) {
        const submittalsData = results[4].value;
        setSubmittalsTotals({
          total: submittalsData.submittals.length,
          waitingforsubmission: submittalsData.submittals.filter(
            (submittal) => submittal.status === "waiting_for_submission"
          ).length,
          inreview: submittalsData.submittals.filter(
            (submittal) => submittal.status === "in_review"
          ).length,
          submitted: submittalsData.submittals.filter(
            (submittal) => submittal.status === "submitted"
          ).length,
          closed: submittalsData.submittals.filter(
            (submittal) => submittal.status === "closed"
          ).length,
        });
      }

      if (results[5].status === "fulfilled" && results[5].value) {
        setFederatedModel(results[5].value);
      }
    })
    .catch((err) => {
  ...
  const totalsSliderItems = [
    ...
    },
    {
      ...
      value: submittalsTotals.total,
      type: "total_submittals",
      description: `${submittalsTotals.waitingforsubmission} waiting, ${submittalsTotals.closed} closed`,
    },
    {
      label: "Waiting Submittals",
      value: submittalsTotals.waitingforsubmission,
      type: "waiting_submittals",
    },
    {
      label: "In Review Submittals",
      value: submittalsTotals.inreview,
      type: "inreview_submittals",
    },
    {
      label: "Submitted Submittals",
      value: submittalsTotals.submitted,
      type: "submitted_submittals",
    },
    {
      label: "Closed Submittals",
      value: submittalsTotals.closed,
      type: "closed_submittals",
    },
        ...
        <>
          ...
          <div className="flex flex-col lg:flex-row h-auto lg:h-[650px] w-full gap-4">
            ...
            <div className="flex space-x-4 mt-4 w-full lg:w-1/5 min-w-0">
              <div className="w-full h-[650px] bg-white rounded shadow p-4 flex flex-col">
                <h2 className="text-lg mb-1 font-semibold">Project Stats</h2>
                <hr className="my-4 border-t border-gray-300" />
                <div className="bg-white overflow-hidden h-[620px] lg:min-h-50">
                  {totalsSliderItems.length > 0 ? (
                    <Slider {...projectSliderSettings(totalsSliderItems)}>
                      {totalsSliderItems.map((item, index) => (
                        <div key={index} className="p-1 h-[125px]">
                          <TotalsCard
                            title={item.label}
                            value={item.value}
